...
const SendDocumentation = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    let email = {
      ...
      message: `Estimado ${CapitalizeNames(
        lead.name
      )}, aquí le hago llegar toda la información del ${CapitalizeNames(
        lead.vehicle.make.name
      )} ${CapitalizeNames(lead.vehicle.model)} ${
        lead.year
      } del cual está usted interesado.`,
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "white", paddingTop }}>
      ...
      <ScrollView>
        <Layout style={{ paddingHorizontal: 15, paddingVertical: 10 }}>
          <Documents
            currents={currentDocuments}
            setCurrents={setCurrentDocuments}
          />
        </Layout>
        <Layout style={{ paddingHorizontal: 15, paddingVertical: 10 }}>
          <Input
        ...
        <Layout style={{ paddingHorizontal: 15, paddingVertical: 10 }}>
          <Input
            disabled
            multiline={true}
            placeholder={`Estimado ${CapitalizeNames(
              lead && lead.name
            )}, aquí le hago llegar toda la información del ${CapitalizeNames(
              lead && lead.vehicle && lead.vehicle.make.name
            )} ${CapitalizeNames(lead && lead.vehicle && lead.vehicle.model)} ${
              lead.year
            } del cual está usted interesado.`}
