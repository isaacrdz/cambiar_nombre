...
const LeadDetailInfoTop = ({ item, loading }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      if (
        ...
        item.substatus &&
        !(
          item.substatus._id === "60ae5d5f60638f03e9c629ce" ||
          item.substatus._id === "605cbaafd5fc4809e161c526"
        )
      ) {
        if (
          user &&
        ...
        if (
          ...
          item.comments.map((comment) =>
            aux.push(moment(comment.createdAt).format("DD MMMM YYYY"))
          );
  ...
  const handleRecover = async () => {
    await updateLead(
      {
        status: "605bd4e80a4330245535db3c",
        substatus: "605bd5e1bed49524ae40f883",
        isContacted: true,
      },
      item._id
    );
